...
def evaluate_attackers(data: pd.DataFrame, screen: str):
    ...
    if data.shape[0] != 0:
        ...
        for user in set(data['User']):
            ...
            predictions = data.loc[data['User'] == user]['Predictions'].to_list()
...
class Evaluator:
    ...
    def calculate_metrics(self, original_user: str, sets_dict: dict):
        ...
        for idx, data_type in enumerate(['acc', 'gyr', 'swp', 'tap']):
            ...
            trn = sets_dict['trn'][idx]['Predictions'].to_list()
            tst = sets_dict['tst'][idx]['Predictions'].to_list()
            att = sets_dict['att'][idx][['User', 'Predictions']]
