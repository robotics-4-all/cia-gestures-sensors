    ...
    for User in tqdm(DF_Users['User'].values):
        ...
        for i in range(len(List_Of_TS)):
            ...
            for j in range(len(S_Of_TSs[i])):
                Screen = S_Of_TSs[i][j]
                Data_Acc = DF_Acc.loc[(DF_Acc['User'] == User) & (DF_Acc['TS'] == TS) & (DF_Acc['Screen'] == Screen)][Feature].values
                Data_Gyr = DF_Gyr.loc[(DF_Gyr['User'] == User) & (DF_Gyr['TS'] == TS) & (DF_Gyr['Screen'] == Screen)][Feature].values  
