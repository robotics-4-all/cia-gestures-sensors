...
#    Functions    #
# =============== #
def CreateUsersDFFsList(UsersList, DFF):
    """
    Separate users data in different dataframes.

    :param UsersList: A list with users names
    :param DFF: A dataframe
    :return: A list with users data dataframes
    """

    DFF_Users_List = []
    for User in UsersList:
        DFF_User = DFF.loc[DFF['User'] == User]
        DFF_User = DFF_User.reset_index(drop=True)
        DFF_Users_List.append(DFF_User)

    return DFF_Users_List
