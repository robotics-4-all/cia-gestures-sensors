...
def Create_DF_Sensors(SensData_Path, DF_Users):
    ...
    :param DF_Users: A dataframe that returned from the findUsers_Common function in s1_Funcs_ExploreData_v000
    ...
    for User in tqdm(Users, desc='Creating Sensors Data Frames'):
        ...
        for i in range(len(List_Of_TimeStamps)):
            ...
            with open(os.path.join(SensData_Path, User + '_' + TimeStamp + '.json')) as jsonFile:
                ...
                for j in json_text['accelerometer']:
                    if j['screen'] in Screens_Of_TimeStamps[i]:
                        ...
                        df = {'User': User, 'TimeStamp': List_Of_TimeStamps[i], 'Screen': Screen, 'X': X, 'Y': Y,
                              'Z': Z, 'Magnitude': Magnitude, 'Combine_Angle': Combine_Angle}
