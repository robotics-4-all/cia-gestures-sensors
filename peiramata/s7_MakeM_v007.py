...
for OriginalUser in DF_Users_Final['User']:
    ...
    EvaluationMetrics_Sensors1 = EvaluationMetrics()
    ...
    for Fold in tqdm(range(k), desc = '-> Fold'):
        ...
        DFF_Sensors1_Org_Trn_X = pd.concat([DFF_Acc_Org_Trn_X, DFF_Gyr_Org_Trn_X], axis=1)
        DFF_Sensors1_Org_Trn_Y = DFF_Acc_Org_Trn_Y
        DFF_Sensors1_Org_Tst_X = pd.concat([DFF_Acc_Org_Tst_X, DFF_Gyr_Org_Tst_X], axis=1)
        DFF_Sensors1_Org_Tst_Y = DFF_Acc_Org_Tst_Y
        DFF_Sensors1_Att_X = pd.concat([DFF_Acc_Att_X, DFF_Gyr_Att_X], axis=1)
        DFF_Sensors1_Att_Y = DFF_Acc_Att_Y
        
        
        # ==================== #
        #    Select Features   #
        # ==================== #
        corr_Sensors1 = DFF_Sensors1_Org_Trn_X.loc[:, Features_Acc + Features_Gyr].corr()
        
        if x == 1:
            plt.figure(figsize=(8, 8))
            corrplot(corr_Sensors1, size_scale=300)
            x = 0
        
        features_corr = pd.DataFrame(columns= ['Feature', 'sum_corr'])
        for i in range(len(Features_Acc + Features_Gyr)):
            sum_corr = 0
            for j in range(len(Features_Acc + Features_Gyr)):
                sum_corr = sum_corr + abs(corr_Sensors1.iloc[i, j])
            df = {'Feature': (Features_Acc + Features_Gyr)[i], 'sum_corr': sum_corr}
        ...
        Features_Sensors1 = list(features_corr['Feature'])
        ...
        inliers = pd.DataFrame(columns = Features_Acc + Features_Gyr)
         
        clf = LocalOutlierFactor(n_neighbors=5, contamination=0.1)		# Find inliers and outliers of one user's swipes
        y_pred = clf.fit_predict(DFF_Sensors1_Org_Trn_X)
        for i in range(len(y_pred)):
        	if(y_pred[i]==1):
        		inliers = inliers.append(DFF_Sensors1_Org_Trn_X.iloc[i], ignore_index=True)
        
        DFF_Sensors1_Org_Trn_X = inliers
        DFF_Sensors1_Org_Trn_Y = DFF_Sensors1_Org_Trn_Y[0:len(inliers)]
        """            
        ...
        Clfs_Sensors1 = []
        maxDistance_Clfs_Sensors1 = []  
        Classifier_Sensors1 = 'LocalOutlierFactor'
        ns_neighbors = [3, 7]
        for n_neighbors in ns_neighbors:
            Parameters = [n_neighbors]
            Clf, maxDistance = Clf_Train(Classifier_Sensors1, Parameters, DFF_Sensors1_Org_Trn_X)
            Clfs_Sensors1.append(Clf)
            maxDistance_Clfs_Sensors1.append(maxDistance)
        ...
        Decisions_Clfs_Sensors1_Org = []
        for k in range(len(Clfs_Sensors1)):
            Decisions_Sensors1_Org = Clf_GetDecisions(Clfs_Sensors1[k], DFF_Sensors1_Org_Tst_X, maxDistance_Clfs_Sensors1[k])
            Decisions_Clfs_Sensors1_Org.append(Decisions_Sensors1_Org)
        Predictions_Sensors1_Org, Decisions_Sensors1_Org = Clf_GetPredictions(Decisions_Clfs_Sensors1_Org)        
        ...
        Decisions_Clfs_Sensors1_Att = []
        for k in range(len(Clfs_Sensors1)):
            Decisions_Sensors1_Att = Clf_GetDecisions(Clfs_Sensors1[k], DFF_Sensors1_Att_X, maxDistance_Clfs_Sensors1[k])
            Decisions_Clfs_Sensors1_Att.append(Decisions_Sensors1_Att)
        Predictions_Sensors1_Att, Decisions_Sensors1_Att = Clf_GetPredictions(Decisions_Clfs_Sensors1_Att)        
        ...
        DFF_Sensors1_Trn_Y = DFF_Sensors1_Org_Trn_Y
        DFF_Sensors1_Tst_Y = DFF_Sensors1_Org_Tst_Y.append(DFF_Sensors1_Att_Y, ignore_index = True)
        Predictions_Sensors1 = Predictions_Sensors1_Org + Predictions_Sensors1_Att
        EvaluationMetrics_Sensors1 = Clf_Evaluate(DFF_Sensors1_Trn_Y, DFF_Sensors1_Tst_Y, Predictions_Sensors1, EvaluationMetrics_Sensors1)        
    ...
    EvaluationMetricsMean_Sensors1 = EvaluationMetrics()
    EvaluationMetricsMean_Sensors1 = CalculateMeanMetrics(EvaluationMetrics_Sensors1, EvaluationMetricsMean_Sensors1)
    EvaluationMetrics_Sensors1 = EvaluationMetricsMean_Sensors1
    ...
    SumaryTable.add_row(['Sensors1', EvaluationMetrics_Sensors1.getTrnOrgSize()[0], EvaluationMetrics_Sensors1.getTstOrgSize()[0], EvaluationMetrics_Sensors1.getTstAttSize()[0], str(round(EvaluationMetrics_Sensors1.getAccuracy()[0]*100,pre))+' %', str(round(EvaluationMetrics_Sensors1.getF1Score()[0]*100,pre))+' %', str(round(EvaluationMetrics_Sensors1.getFAR()[0]*100,pre))+' %', str(round(EvaluationMetrics_Sensors1.getFRR()[0]*100,pre))+' %'])    
