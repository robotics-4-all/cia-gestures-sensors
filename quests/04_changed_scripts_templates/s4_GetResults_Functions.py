...
def get_results(case: str, screen: str, screen_path: str,
                ...
                nu, gamma) -> pd.DataFrame:

    print(' - Getting results.')
    path_df = os.path.join(screen_path, 'results_' + str(nu) + '_' + str(gamma) + '.csv')
    ...
    for original_user in tqdm(set(df_acc['User'])):
        ...
        for data_type in ['acc', 'gyr', 'swp', 'tap']:
            classifiers.append(dict_cases[case]['GetResults']['Classifiers'][data_type](nu, gamma))
