...
class AccClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        parameters = []
        for nu in np.arange(0.2, 0.35, 0.01).round(2).tolist():
            for gamma in np.arange(0.03, 0.05, 0.001).round(5).tolist():
...
class GyrClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        clf_name = 'OneClassSVM'
        parameters = []
        for nu in np.arange(0.4, 0.7, 0.02).round(2).tolist():
            for gamma in np.arange(0.15, 0.25, 0.005).round(3).tolist():
                parameters.append([gamma, nu])
        clfs_parameters = {clf_name: parameters}
...
class SwpClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        clf_name = 'OneClassSVM'
        parameters = []
        for nu in np.arange(0.3, 0.5, 0.01).round(2).tolist():
            for gamma in np.arange(0.15, 0.25, 0.005).round(3).tolist():
                parameters.append([gamma, nu])
        clfs_parameters = {clf_name: parameters}
...
class TapClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        clf_name = 'OneClassSVM'
        parameters = []
        for nu in np.arange(0.7, 0.75, 0.01).round(2).tolist():
            for gamma in np.arange(0.5, 0.9, 0.01).round(2).tolist():
                parameters.append([gamma, nu])
        clfs_parameters = {clf_name: parameters}
