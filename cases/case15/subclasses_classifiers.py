...
class AccClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        parameters = []
        for nu in np.arange(0.01, 0.04, 0.01).round(2).tolist():
            for gamma in np.arange(0.00005, 0.00090, 0.00005).round(5).tolist():
...
class GyrClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        clf_name = 'OneClassSVM'
        parameters = []
        for nu in np.arange(0.12, 0.26, 0.02).round(2).tolist():
            for gamma in np.arange(0.00040, 0.00400, 0.00025).round(5).tolist():
                parameters.append([gamma, nu])
        clfs_parameters = {clf_name: parameters}
...
class SwpClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        clf_name = 'OneClassSVM'
        parameters = []
        for nu in np.arange(0.10, 0.15, 0.01).round(2).tolist():
            for gamma in np.arange(0.00040, 0.00400, 0.00025).round(5).tolist():
                parameters.append([gamma, nu])
        clfs_parameters = {clf_name: parameters}
...
class TapClassifier(Classifier):
    ...
    def __init__(self, param):
        ...
        for nu in np.arange(0.40, 0.54, 0.01).round(2).tolist():
            for gamma in np.arange(0.500, 0.800, 0.025).round(3).tolist():
                parameters.append([gamma, nu])
        clfs_parameters = {clf_name: parameters}
