...
from s5_Funcs_HandleClassifiers_v001 import AppendUserMetrics, CalculateMeanMetrics, Clf_Train, Clf_GetDecisions, Clf_GetPredictions, Clf_GetTotalPredictions, Clf_Evaluate
...
for idx_OrgU in range(len(DF_Users_Sensrors)):
    ...
    for Fold in tqdm(range(10), desc = '-> Fold'):
        ...
        for k in range(len(Clfs_Acc)):
            ...
            Predictions_Acc_Org_Tst = Clf_GetPredictions(Decisions_Acc_Org_Tst)    
        ...
        for k in range(len(Clfs_Gyr)):
            ...
            Predictions_Gyr_Org_Tst = Clf_GetPredictions(Decisions_Gyr_Org_Tst)    
        ...
        for k in range(len(Clfs_Swipes)):
            ...
            Predictions_Swipes_Org_Tst = Clf_GetPredictions(Decisions_Swipes_Org_Tst)    
        ...
        for k in range(len(Clfs_Acc)):
            ...
            Predictions_Acc_Att = Clf_GetPredictions(Decisions_Acc_Att)    
        ...
        for k in range(len(Clfs_Gyr)):
            ...
            Predictions_Gyr_Att = Clf_GetPredictions(Decisions_Gyr_Att)    
        ...
        for k in range(len(Clfs_Swipes)):
            ...
            Predictions_Swipes_Att = Clf_GetPredictions(Decisions_Swipes_Att)    
...
Experiment Description:
    ...
    - Majority voting where needed with precitions !!!! <--------------------
...
|        |   ize   |   ize   |   ize   |    y    |         |         |         |
+========+=========+=========+=========+=========+=========+=========+=========+
|  Acc   | 217.714 | 53.857  | 5431.42 | 79.297  | 16.905  | 20.866  | 6.746 % |
|        |         |         |    9    |    %    |    %    |    %    |         |
+--------+---------+---------+---------+---------+---------+---------+---------+
|  Gyr   | 217.714 | 53.857  | 5431.42 | 68.913  | 9.422 % | 31.277  | 12.746  |
|        |         |         |    9    |    %    |         |    %    |    %    |
+--------+---------+---------+---------+---------+---------+---------+---------+
| Swipes | 320.286 | 79.524  | 7996.19 | 98.823  | 6.942 % | 0.235 % | 94.683  |
|        |         |         |    0    |    %    |         |         |    %    |
+--------+---------+---------+---------+---------+---------+---------+---------+

Majority voting where needed with precitions, have similar results FAR-UP FRR-DOWN
"""
