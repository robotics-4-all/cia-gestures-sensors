...
def Create_DFF_Swipes(DF_Gest, Normalize):
    ...
    :param DF_Gest: A dataframe that returned from the Create_DF_Gestures function in s2_Funcs_CreateDataFrames_v001
    :param Normalize: If True normalize gestures data in a specific screen size
...
def FeatureExtraction_Sensors(User, TimeStamp, Dataset, Window, Overlap, FeautureObject):
    """
    Extract features from a sensors data window.
    The window contains data from the same timestamp.
...
def Create_DFF_Sensors(DF_Users, DF_Acc, DF_Gyr, Feature, Window, Overlap):
    ...
    :param DF_Users: A sensors dataframe that returned from the findUsers_Common function in s1_Funcs_ExploreData_v001
    :param DF_Acc: A dataframe that returned from the Create_DF_Sensors function in s2_Funcs_CreateDataFrames_v001
    :param DF_Gyr: A dataframe that returned from the Create_DF_Sensors function in s2_Funcs_CreateDataFrames_v001
    ...
    for User in tqdm(DF_Users['User'].values):
        ...
        for i in range(len(List_Of_TS)):
            TS = List_Of_TS[i]
            Data_Acc = DF_Acc.loc[(DF_Acc['User'] == User) & (DF_Acc['TS'] == TS)][Feature].values
            Data_Gyr = DF_Gyr.loc[(DF_Gyr['User'] == User) & (DF_Gyr['TS'] == TS)][Feature].values
            F_Acc = FeatureExtraction_Sensors(User, TS, Data_Acc, Window, Overlap, F_Acc)
            F_Gyr = FeatureExtraction_Sensors(User, TS, Data_Gyr, Window, Overlap, F_Gyr)
